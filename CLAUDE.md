...
페이지 조회는 동기 메모리 조회다. 작업 중 중복 편집을 거절하고 종료 시 저장 완료를 기다린다.
실제 SMB의 블로킹 호출을 강제 종료하지 않는다. 최신 AI_HANDOFF/REV1_IMPLEMENTATION 참고.

### 최신 재개: 공유 문서 행 추가 (2026-09-22 KST)

`eed56a2`에서 이어 기존 공유 문서 마지막 행 추가를 새 UI에 연결했다.
제한 IPC `document_append(snapshot, values)`, 기존 잠금/변경 검사와 원자 교체를 재사용한다.
기존 숨김 자격증명 열을 노출/복제하지 않는다. 상세 검증과 잔여 작업은 최신 인수인계를 읽는다.
...
- WPH 화면은 마지막 조사 시작의 호기/검색어만 복원한다(wph_last_investigation).
  폴더 경로로 자동 선택하지 않는다. 검색 이력을 누적하지 않고 조사 시작 때 교체 저장.

- 축 표시를 명시하고 시간축에 약 5개 날짜·시간 눈금을 배치한다.
  항목명은 축, 단위 포함 수치는 막대 위에 표시해 의미와 가독성을 함께 유지한다.

- 최신 차트: 34cm 폭 세로 배치, 단위 포함 수치 라벨.
  오류 종류는 분할하지 않고 지표별 한 차트에 모두 표시(2026-09-16 수정).
  항목 수에 맞춰 높이/배치 간격을 늘리고 숫자축은 0부터 오른쪽으로 증가,
  정수 눈금 간격을 최소 1로 지정한다. Report 수와 Wafer/Slot 건수는 각각 유지.
  WPH/Batch 소요시간은 Report 내부 Batch End 순서 및 실제 시간 X축을 사용한다.
  시간 미확인 건은 추이에서 제외·건수 표시하며 분포/누적 집계에는 계속 반영한다.
...
- 기존 WPH 계산식과 조사 파일 선택·로컬 출력 원칙 유지. 추가 장비 읽기 없음.

### 파라미터 UX·색상 (2026-09-14)

- 색상 코드 입력과 함께 실제 색상표·미리보기 제공. 양식 하단 ‘색상 보고 선택…’은
  선택한 여러 파라미터에 일괄 적용한다. 행별 위젯 없이 캔버스 셀 배경으로 색 표시.

- version_new에서만 작업. 변환계수 창 스크롤, 왼쪽 장비풍 회색 표시/색상 칸,
  오른쪽 비교 구조 유지, 비고 셀 위치 수정. 장비 값 조절 슬라이더는 넣지 않는다.
- 장비화면이름.xlsx 마지막 열 색상코드 추가. (Alg, 원본항목) 키로 재사용한다.
  양식은 색상코드 텍스트 편집·확정 저장, 값 확인은 색상 칸 더블클릭·명시적 저장.
- 상세 사용법/검증/구버전 쓰기 호환 한계: docs/UX_PARAMETER_COLORS.md.
- 새 색상 기능도 기존 장비 읽기 전용·OneDrive 쓰기 제한을 지킨다.

### version_new 검토 (2026-09-13)

- 이번 작업과 다음 인수인계는 `version_new`만 사용한다. 원본·다른 브랜치는 수정 금지.
- 변경 목록·검증 범위·OneDrive 한계·Windows 실기 항목은 `docs/REVIEW_VERSION_NEW.md`.
- 공유 폴더 임시 JSON 추가안은 철회. atomicfile은 로컬 사용자 설정에만 적용한다.
- 장비 읽기 전용, net use 미사용, 접속 간격, 로컬 staging, 공유 쓰기 최소화 유지.
- WPH 초과 행 수식/선택, Commonality 대기 폴더, GUI 작업 콜백, 업데이트 백업을 수정.
- 같은 버전의 다른 exe 덮어쓰기 게시 금지. 실제 신규 배포는 8.0보다 높은 버전으로 빌드.
- 과거 문서의 '완전 오프라인/네트워크 불필요'는 앱 직접 인터넷 호출이 없다는 의미로
  제한한다. 장비 SMB와 사용자의 OneDrive 클라이언트 동기화는 별개로 필요하다.
...
- **흐름(GUI 2단계)**: ①호기 다중 선택 + **호기별 Report 폴더 직접 지정**(config
  `wph_report_paths`, 탐색기로 미리 연결) + **recipe 검색어 입력→[검색]으로 개수 표시**.
  · **검색 = 접두가 아니라 포함(substring, 2026-09 변경)**: 검색어가 파일 이름에
    **포함**되면 매칭(`_matches`, 대소문자·주변공백 무시·한글 보존). 공백으로 나눈
    여러 단어는 **모두 포함**(AND·순서 무관). 이름만 보므로 빠름·원본 무접근.
  · **[📋 더보기] 리포트 선택창(2026-09)**: 검색 결과를 실제 목록으로 보여주고
    체크박스로 조사할 batch report 를 고른다(`_wph_more`). **기본 전체 선택** ·
    전체 선택/전체 해제 · ↑↓ 이동 · Enter/Space 토글 · 클릭 토글. 확인하면 개수
    라벨이 `선택 N / 발견 M` 로 바뀌고, 조사에는 **체크한 리포트만** 쓴다
    (세션 상태 `_wph_rows[호기]["all_names"]`·`["selected"]`, `collect_rows(names=)`).
  1개 이상 선택된 호기만 대상. ②**유효 Lot 매수**(기본 25, config `wph_valid_wafers`,
...
`장비 IP 주소/참고자료/특이사항/변환계수/장비화면이름.xlsx`(5) · `감시설정.json` ·
...
- **적용**: 게시된 exe 를 **로컬 Temp** 로 복사(OneDrive 에 쓰지 않음) →
  ...
  **사용자는 확인 한 번만, 재시작은 전부 자동.** 실행 중인 exe 는 자기 자신을 못
  ...
  · **업데이트 종료 = 강제 종료(2026-09)**: 교체 스크립트는 **구 exe 잠금 해제**를
    종료 신호로 쓰므로, 트레이 스레드 등이 남아 exe 를 계속 잠그면 2분 대기 후
    '옛 버전'을 되살려 **새 버전이 자동 실행되지 않는** 증상이 있었다. 그래서
    업데이트로 종료할 때는 `_run_update` 가 `self._updating=True` 를 세우고
    `_shutdown` 이 정리 후 **`os._exit(0)`** 로 즉시 종료해 exe 를 곧바로 풀어 준다
    (스크립트 `:gone` 분기가 바로 새 target 을 실행). 일반 종료는 종전대로.
