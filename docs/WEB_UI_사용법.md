...
|---|---|
| 취합 비교·색상/비고·셀 색칠·내보내기·레시피 삭제, 이력 diff·Excel | 파라미터 자동 감시(무인 수집)·Commonality 자동 감시 |
| 값 업데이트(장비/로컬 수집 → 취합) | 트레이 상주 |
...
※ 웹 엔진과 기존 프로그램은 **동시에 켤 수 없습니다**(같은 단일 실행 잠금 사용). 자동 감시가 필요하면
기존 프로그램을, 그 밖의 작업은 웹 UI를 사용하세요. 실제 장비·Windows 실기 검증은 아직 필요합니다.
