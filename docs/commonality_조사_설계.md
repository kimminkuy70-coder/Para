...
- **Lot 계획 엑셀 템플릿을 프로그램이 생성**해서 사용자가 수정하도록 띄운다.
  - 열: `1=디바이스명, 2=LOT번호, 3=S/M, 4=AOI호기`.
- 업로드 → **선택한 호기 행만** 필터.
- 각 행을 실제 폴더로 해석: 디바이스명(→ `2D@..` 폴더 매칭) + LOT번호(→ `{LOT}` 레벨)
  + S/M(→ `{S/M}` 레벨). **폴더 실재 확인**.
...
- 레이아웃(사용자 지정): `1열=LOT번호, 2열=장비 호기, 3열~=파라미터`.
  → 각 **행 = (LOT, 호기)**, 열 = 파라미터(Step 5의 전치 형태).
...
- **추천/구현안**: tksheet 격자 뷰어
  ...
  - 파라미터별 이탈 요약(고유값 개수 · 어떤 LOT/호기가 다른지),
...
- `param_manager/commonality.py` — 헤드리스 코어:
  - Lot 계획 엑셀 **템플릿 생성/판독**(`디바이스명/LOT번호/S/M/AOI호기`),
  - 호기 필터 + Lot 폴더 해석(디바이스명·LOT·S/M 매칭, 실재 확인),
  - **이름순 첫 웨이퍼** 선택,
  - Lot별 값 추출(`ini_parser` 재사용) + **구조 diff**(Lot 간 설정키 비교),
  - 호기 결과 엑셀 write/read,
  - **호기 취합·비교**(과반수 대비 이탈 색칠, LOT/호기/파라미터 레이아웃).
...
{저장폴더}/commonality/{호기}/{생성시간}/
    staging/{LOT}/{Zones|RTP.txt|OpticPreset.ini}   # Step3 복사본
