...
1. **IP↔Scanresult 루트**: **호기별 루트를 config(`commonality_roots`)에 저장** — 첫 사용
   ...
   **백업본이 여러 개면(Scanresult / Scanresult_260402 / SCANRESULT_BACKUP_260805 등)
   호기 폴더만 지정하면 그 아래 Scanresult* 를 전부 탐색해 Lot 을 찾는다**
   (`scanresult_roots`; resolve_* 가 여러 루트를 모두 검색). Scanresult 폴더 하나만
   직접 골라도 됨(그 폴더만).
