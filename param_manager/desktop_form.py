...
from . import (coefstore, editor_model, engine, extract_io, formbuilder,
               ini_parser, locking, namestore, workdirs)
...
class DesktopForm:
    ...
    def catalog(self):
        ...
        return dict(save_dir=True, recipes=recipes)
    ...
    def confirm(self, params):
        ...
        for e in self.entries:
            variant = e["variant"]
            # Coefficient for LINEAR/AREA: human-managed coefstore first, else the
            # coefficient already embedded in the candidate's transform label, else default.
            coef = coefstore.lookup(coef_rows, machine, variant) if coef_rows else None
            if coef is None:
                coef = ini_parser.scale_from_label(engine._s((e["ext"] or {}).get("transform")))
            if coef is None:
                coef = ini_parser.DEFAULT_SCALE
            selected.append(dict(use=e["use"], name=e["name"], reco=e["reco"],
                                 variant=variant, zone=e["zone"], alg=e["alg"],
                                 ext=e["ext"], method=e["label"], coef=coef))
